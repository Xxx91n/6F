...
- **[CodeLore](https://github.com/emrecdr/codelore)** — 代码考古／证据层外部 CLI（explain/summary 只读探针面）
- **[DuckDB Node API](https://github.com/duckdb/duckdb-node)** — 各审计尺度事实表写路径底座（@duckdb/node-api）
- **[DuckDB Node bindings](https://github.com/duckdb/duckdb-node)** — 事实表运行所依赖的平台原生 DuckDB 二进制（@duckdb/node-bindings-*）
- **[git](https://github.com/git/git)** — 确定性仓库考古与证据采集（宿主环境外部 CLI）
- **[GitHub REST API](https://docs.github.com/rest)** — Micro-A PR 数据面——枚举／元数据／diff 兜底（本地 git 优先）
- **[esbuild](https://github.com/evanw/esbuild)** — 打包发行用单文件 dist bundle 构建器
- **[SchemaStore](https://github.com/SchemaStore/schemastore)** — claude-code-plugin-manifest schema（快照即契约的本地校验源）
- **[Claude Code CLI](https://github.com/anthropics/claude-code)** — advisory 位的 claude plugin validate 校验工具链
...
- **badge 只挂当下为真项**：license／version／Node 下限／marketplace 可安装／engine-ci main 分支状态徽记（2026-09-22 首个 main 绿 run 后挂载——event_bound，不预埋）。动图待 listing-material-freeze 后拍真实 UI——存在前一律不引用。

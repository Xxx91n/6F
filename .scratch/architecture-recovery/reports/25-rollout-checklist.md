...
| B3.3 | 仓根 CHANGELOG.md 记录本轮 7 ADR + 18 A-xxx | 能——但 BACKLOG 口径已过期：现应为 14 ADR + A-001~A-030 + R3 执行轮 | 需：确认 CHANGELOG 口径（更新后范围） | decided-now（R7-D-039）：双层分工——engine/CHANGELOG.md=产品版本账唯一权威（KaC）；仓根 CHANGELOG.md=里程碑编年指针制（M-xxx 键/ISO日期，禁版本号头，只引用不复制）；指针有效性守卫并入 #44；仓根编年落盘随 #41a |
...
| P2 | 版本与上游锁定（调研四层闸门②） | 部分能——engine/package-lock.json 在库、版本 0.1.0 无复用冲突；版本单调策略与上游锁定表状态列待制度化 | 需：版本策略 + 上游清单表「已接入/规划中」状态随阶段 2/3 更新（衔接 T2） | decided-now（R7-D-037）：现在拍（升级非改绑）——0.x 单调纪律＋engine/upstream-lock.yaml 制度化（pin_type/status 四态）＋报告契约补强（report_schema/stability/capabilities/superseded_by）＋手动窗口禁自动升级＋advisory→enforce 守卫（#44）；docs/versioning.md 成文 |
...
| D4 | 演示入口（内置样例项目 / demo 数据面） | 不能——未实现 | 需：立票时定义形态（样例仓 or 内置 fixture） | decided-now（R7-D-038）：内置合成 fixture 生成器为主干（fixtures/definitions 首发三场景＋golden/＋generator/ 随 tgz；demo --scenario 临时目录生成）；外部样例仓仅文档 opt-in；CASRAI 式机器可读披露块；归票 #45（#43←#45），列阶段 3 早期票 |
...
> （2026-09-15 R7 收口：P2/B3.3/D4 三行拍板状态列已按 D-037/D-039/D-038 更新为 decided-now；判定列与正文保持原样——触发已发生＋拍板完成只改状态列。）

...
| B1.2 | 启动器收尾硬要求加黑体强提示（防 W2/W3 V2 重演） | 能（18 份启动器文档编辑） | 需：确认强提示措辞与位置（黑体警告样式） | 触发已发生（2b 结题 2026-09-15）→ 待用户拍板（D-026 SLA：1 工作日内）；启动器行为本轮未改，措辞沿用现行 |
...
| B4.2 | 与 jiahao / anysearch-cli / env-manager 对接 | 不能立即（多票工程，视产品方向；三仓被 W2 扫描但未实际集成） | 需：用户定产品方向与对接优先级 | 触发已发生（2b 结题 2026-09-15）→ 待用户拍板（D-026 SLA：1 工作日内） |
...
| P6 | 上架最小形态（能力铺开度口径） | 不能现在定——Macro-B 单 scale 已可跑通（首报闭环）；是否「单 scale 即上架」vs 等更多 scale 属产品裁定 | 需：用户定「最小可发布形态」口径 | 触发已发生（阶段 2 双结题 2026-09-15）→ 待用户拍板（D-026 SLA：1 工作日内） |

## §7 演示资产范围清点（校准 = 调研 §2.3「两条路径逐一对齐」）

| 编号 | 资产项 | 可执行判定 | 需什么 / 前置 | 拍板状态 |
|---|---|---|---|---|
| D1 | 样例报告（审核员路径核心资产） | 能——已有 reports/23-first-report.{md,json} + 23-first-report-failure.{md,json}（同骨架 + ⚠ unverified），含真实感数据且为 6F 自身审计（无敏感数据） | 需：用户确认将其列为发布样例资产 | 触发已发生（阶段 2 双结题 2026-09-15）→ 待用户拍板（D-026 SLA：1 工作日内） |
| D2 | 演示路径覆盖（10 路径法，D-007/ADR-0007） | 部分能——已交付 2/10（Macro-B happy + failure）；其余 8 路径（4 scale × 2）属阶段 3 铺开 | 需：拍板上架期演示口径（仅 2/10 或排期补齐） | 触发已发生（2b 结题 2026-09-15）→ 待用户拍板（D-026 SLA：1 工作日内） |

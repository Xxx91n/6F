...
const measurements = {
  ...
  codelore: {
    pinned: JSON.parse(codeloreFacts[0].value_json).pinned === true,
...
const adjudicationEntries = [
  { criterion_id: 'PC-MC-1', band: pcMc1 ? 'supported' : 'insufficient', basis_refs: ['管线活性正对照'], anchored_fact_ids: [codeloreFacts[0].fact_id].concat(facetFactIds), anchored_evidence_ids: ['EV-38-05'], decided_at: HEAD_DATE, rationale: pcMc1 ? 'codelore pin 0.28.0 + 30/30 facet_rows + ADR 语料 ' + adrs.length + ' 份解析' : '管线活性未中（pin/facet/语料缺）——管线故障 P0' },
  ...
  { criterion_id: 'NC-MC-1', band: nc1Pass ? 'supported' : 'insufficient', basis_refs: ['负对照特异性'], anchored_fact_ids: [nc1Facts[0].fact_id], anchored_evidence_ids: ['EV-38-05'], decided_at: HEAD_DATE, rationale: nc1Pass ? '负对照选材 apps/cli/package.json 五件套 0 命中（特异性成立）' : '负对照命中 → 转复核路径' }
];

...
- **test 闭环**：`npm test` = gen + build + smoke(6/6) + collectors(13/13) + codelore-adapter(7/7) = 26 断言；CI 矩阵 3 OS × Node 20/22 跑同一 `npm run smoke` 链（engine-ci.yml 未改）；纯文档票（#26/#29/#30）以守卫脚本闭环验收
...
- 归属分解：detector-miss 16 格（form 11 + inline 5）/ real-gap 33 格 / post-freeze 16 格
...
- 栈：`8be9db5` ← r6-ticket-scaffold(txv) ← 26-adr-measurement-audit(kpz) ← 27-criteria-v2-fallback-chain(pov,uvr,orr) ← 28-adr-hygiene-sweep(vkn) ← 29-c-disposition-reopen(nqq) ← 30-frozen-calibration-desk(nvm) ← 31-codelore-probe(oyp)
- 未 push；无 PR 创建（发布属用户闸门）。

## §6 T7 值守核对

- #25 拍板状态列：25 行全有状态值（B4.1 行 `—` 规范化为「关闭（D-015 supersede 吸收，非事项）」）；14 行挂门条款绑定「最迟时点 + 触发事件」（D-026）未触发。

// R5 T0 票据包自检 v1 —— 票 #32~#43 三件套齐备 + 启动器硬规则（≤60行/黑体块/违禁词/报告路径）+ 必读清单路径可解析
...
for (const n of NN) {
  ...
  t(slug+' 收尾报告路径', t0.includes('reports/'+n+'-report.md'));
  const issueSlug = fs.readdirSync(AR+'/issues').find(f=>f.startsWith(String(n)+'-'));
  const hoSlug = fs.readdirSync(AR+'/handoffs').find(f=>f.startsWith(String(n)+'-'));

...
| #52a | checker-eval 仪器标定：合成 claim-evidence 对 ~80-120 条按 checker 判定空间分层（presence 成立/不成立、支持/矛盾/中立、band-leak 注入、边界样本）＋precision/recall 分类别＋FP/FN 分型（CiteEval 法）＋band-leak 检出率（BAND_PATTERNS 独立复测）＋κ 基线（50-100 条子集双标→human-human κ 天花板→checker-人 κ，预声明 κ≥0.6＋raw agreement＋bootstrap CI 双报）＋eval 结果 JSON＋NN-check 断言＋合成限制披露 | D-061 | 铺开窗口（即时可执行） | golden claim set 版本化＋更新走 PR 评审（D-037/#43 惯例）；checker 阈值改动不得参照本集标签（防调参泄漏）；held-out 只评一次；**评测票与修复票分离**（暴露缺陷另立票）；不改 checker 行为（D-059⑤ 叠加分级不扩砍） |
...
| #54 | P0 %cI 确定性修复：%cI→%ct（或归一化 +00:00→Z）＋gitcli 锁表行 enforce 输出格式断言（契约从未 enforce 的真缺口）＋golden-ci「单平台即可」声明勘误 | D-059① | **P0** | 直击逐字节确定性核心宣称；traceId→fact_id→receipt→report 链全漂风险 |
| #55 | P1 批票：SQL 黑名单剥字面量后匹配＋误伤回归测试（D-059④，不上 AST）＋MCP db 寻址收敛进 mcp.json 配置/服务端解析（D-059⑥——堵 D-053 叙事主路）＋intake snapshot_fetched_at 时点披露＋显式刷新 opt-in（不自动 pull 保隔离）＋.git 键归一（D-059⑦）＋citation.ts contradicts 死枚举勘误清除（D-059⑤，YAGNI） | D-059④⑤⑥⑦ | **P1** | 三子项可分 PR；mcp.json 面改动须与 #53 audit 票对齐寻址约定 |

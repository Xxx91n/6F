...
| 日期 | 事由 | 影响 | 解决方案 | 引用 |
|---|---|---|---|---|
| 2026-09-26 | 轮36 T1 P4 as-cast 返工（A-094）：①D-147 裁定机制句「const 别名收窄自动兑现」实测不成立——TS 5.9.3 别名收窄对函数内多处赋值的 let 绑定不产生窄化，落地改为 const 三元单式才兑现；②kmk/xtl 式 commit message 缺 A-NNN 引用系两轮同款疏漏再发（P6） | 教训：类型级修法落地前先用最小复现证机制假设（TS 特性适用面≠望文生义）；commit message 收尾自检增「A-NNN 引用在场」一项 | D-147 + A-094 |

...
| A-028 | 「审计/审查类产品"第一份真报告"的专项先例…建议在阶段 1 执行时对同类（CodeScene 行为分析、GitClear）补一轮定向调研」+「mutation…对本产品 B 层的迁移有效性需在阶段 2 用更多仓样本复核」 | 阶段 2 用首报实测锚补 CodeScene/GitClear 定向调研 + B 层迁移有效性多仓复核 | 定向调研只作校准输入不阻塞首报；多仓复核在阶段 2（阶段 1 只跑 6F） | D-016/D-018 | current **→ deferred（收口结算 2026-09-13）** ：票 #24 未开工，下一波候选 |
...
| A-030 | T6「铺开其余采集器/scale + 分发收尾（plugin 上架、BACKLOG 卫生）」——分发收尾前置未清点（BACKLOG B1/B2/B3 立票决定在用户手中、plugin 上架条件、演示资产范围） | 分发收尾前置清单（BACKLOG 立票/上架条件/演示资产）清点落文，逐项待用户拍板 | B4.1 已被 D-015 取代不再立项；用户逐项拍板后才动 | D-016 | current → **deferred（收口结算 2026-09-13：票 #25（铺开与分发收尾）未开工，前置清单待用户逐项拍板）** |

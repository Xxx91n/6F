...
- `D:\Aworker\6F\engine\src\fact\file-card.ts`：`collectRenameEdges` 边池（JSON.parse 防御式 malformed skip）＋`resolveLineageAncestors` 反向 BFS（to→from 邻接表；visited 去重＋dup 命中时 `reachesForward` 前向可达判定真环（DAG 钻石/跨集同键边不置旗）；`FILE_CARD_LINEAGE_MAX_ANCESTORS=64` 帽→truncated 如实披露；BFS 次序注入 stitched_from）＋`FileCardLineage`/`FileCardLineageEdge` 披露块（stitched_from/edges/cycle_detected/truncated）＋facet_rows 行级 `subject_ref` 溯源键（additive）＋`fileCardCitationKeys` 扩展 lineage.* 锚＋peer 分布被取代旧名去重（同一文件新旧名只计新名）＋renamed_to 边池=选中集∪祖先集。
...
- L1 1-switch 成对件新名端（并入+F10 回归=不误进 new_file）；L2 多跳 A→B→C；L3 环检测 A⇄B；L4 跨集注入缝；L5 真 DuckDB 端到端（miss 侧跨集实证=gone.ts 真实断言）；L6 0-switch 四态点名+双对照组（new_file/insufficient_history）；返工增补：L7 跨集 dup 边零假阳／L8 DAG 钻石零假阳／L9 真环对照置旗／L10 EDGE_CAP 截断披露（FILE-CARD 36/36）。
...
- 预登记票面：`D:\Aworker\6F\.scratch\macro-audit\reports\80-bench-thresholds.md`；实测件 `audits/r36/bench-file-card.json` 读数（逐跑有 host 噪声，以工件为准）：jiahao p95=112.63（1105 行 small）/ env-manager p95=89.61（680 行 small）/ 合成 20561 行 medium p95=41.35；verdict 全 target。
...
| 守卫 | 33 39 40 41a 43 44 45 70 71 72 73 77 78 80 81 82 83 41b xfail | 全 rc=0（41a 38/38・44 59/59・77 16/16・80 28/28・83 19/19） |

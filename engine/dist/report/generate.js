...
export function chapterHeading(ch) {
    return '## ' + ch.id + ' ' + ch.name;
}
export function skeletonOf(markdown) {
    const out = [];
    ...
    for (const line of lines) {
        ...
        if (id.length !== 2 || id.charAt(0) !== 'C') {
            continue;
        }
        if (id.charAt(1) < '1' || id.charAt(1) > '9') {
            continue;
        }
...
export function renderSidecar(r) {
    return JSON.stringify(toSidecar(r), null, 2);
}
export function degradeReport(r, reason) {

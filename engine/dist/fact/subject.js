...
export function normalizeSubjectPath(raw) {
    const warnings = [];
    ...
    if (s.indexOf(String.fromCharCode(0)) >= 0) {
        return { ok: false, subject: null, reason: 'nul-byte', warnings: warnings };
    }
    if (ABSOLUTE_RE.test(s)) {
        return { ok: false, subject: null, reason: 'absolute', warnings: warnings };
    }
    ...
    for (const seg of segs) {
        if (seg === '..') {
            return { ok: false, subject: null, reason: 'dotdot-segment', warnings: warnings };
        }
        if (seg === '') {
            if (segs.length > 1) {
    ...
    if (kept.length === 0) {
        return { ok: false, subject: null, reason: 'empty-after-normalize', warnings: warnings };
    }
    let out = kept.join('/');
...
export function detectCaseOnlyConflicts(subjects) {

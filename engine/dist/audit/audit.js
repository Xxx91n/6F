...
export function normalizeAuditScale(raw) {
    if (raw === undefined) {
        return 'Macro-B';
    }
...
export function auditRepoName(input, resolvedRoot) {
    const t = input.trim();
...
const AUDIT_INTENT_CANDIDATES = ['CONTEXT.md', 'README.md', 'AGENTS.md'];
// NC-1 负对照候选集=39 同款宽面（跨生态非 ADR 文件，五件套预期 0 命中）
const AUDIT_NC1_CANDIDATES = ['package.json', 'README.md', 'README.adoc', 'README.rst', 'README', 'Cargo.toml', 'pom.xml', 'build.gradle', 'LICENSE', 'LICENSE.txt', 'pyproject.toml', 'go.mod', 'Makefile'];
...
export async function runAudit(opts) {
    ...
    const bhvTc1 = bhvRan && hRows.length > 0 && hRows.every(function (r) { return typeof r.revisions === 'number' && r.revisions >= BHV_MIN_REVS; });
    const bhvTc2 = bhvRan && cRows.length > 0 && (cRows.filter(function (r) { return r.shared >= 2 && r.degree > 0; }).length / cRows.length) >= 0.5;
    ...
    const measurements = {
        ...
        collection_environment: {
            ...
            instrument_dialect: {
                ...
                by_rule: probes.dialectAbsorptions.reduce(function (m, e) { m[e.rule_id] = (m[e.rule_id] || 0) + 1; return m; }, {}),
                by_field: probes.dialectAbsorptions.reduce(function (m, e) { m[e.field_name] = (m[e.field_name] || 0) + 1; return m; }, {}),

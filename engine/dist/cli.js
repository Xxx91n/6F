...
const outExit = (text, code) => { process.stdout.write(text, function () { process.exit(code); }); throw new ExitSignal(code); };
const errExit = (text, code) => { process.stderr.write(text, function () { process.exit(code); }); throw new ExitSignal(code); };
async function main() {
    ...
    if (cmd === '--version' || cmd === '-v') {
        ...
        try {
            ...
            const r = await runAudit({ input: input, scale: scale, outDir: outDir, json: asJson, refresh: refresh, strictQuarantine: strictQuarantine });
            if (r.out_dir) {

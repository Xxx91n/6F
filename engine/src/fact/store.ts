...
const NO_OFFICIAL_BINDINGS = new Set<string>(['win32-arm64']);
...
function emitSelfHeal(ev: Record<string, unknown>): void {
  ...
  if (process.env.MACRO_AUDIT_MCP_STDIO === '1') console.error(line); else console.log(line);
}
...
function loadDuckdb(): Promise<DuckDBModule> {
  if (!duckdbModulePromise) {
    duckdbModulePromise = import('@duckdb/node-api').catch(async function (e) {
      ...
      if (heal.ok) {
        try { return createRequire(import.meta.url)('@duckdb/node-api') as DuckDBModule; } catch (e2) { e = e2; }
      }
      duckdbModulePromise = null;
      throw new Error('DUCKDB-UNAVAILABLE: 自动补拉失败（' + heal.detail + '）；在插件目录手动执行 `npm install --omit=dev` 恢复 facts/audit 读写面；无网络环境下 facts/audit 不可用、其余命令不受影响（原始解析错误：' + String(e && (e as Error).message || e) + '）');
    });
  }

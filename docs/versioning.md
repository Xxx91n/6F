...
- 种子行（#44 落盘）：codelore=active（exact-version＋`--version` pin 契约）／scorecard=planned、repomix=retired（D-056，重开触发器登记于 registry）／sqlite-dump=evaluating（风险注记）；github-rest=planned（remote-api kind，#47/D-048）。
